...
%             folder or not
            ...
            %% Count the number of samples taken in one day
%             for i=1:length(theFiles)
%                 baseFileName = theFiles(i).name;
%                 fullFileName = fullfile(directory, baseFileName);
%                 rowsNum = rowsNum + numel(textread(fullFileName,'%1c%*[^\n]')); %Count the number of samples by reading the first character of each file
%             end
            %%
%             samples = uint8(zeros(rowsNum, 6));
            samples = uint8(zeros(0, 6));
            sampleCounter = 1;
            for i = 1:length(theFiles)
                baseFileName = theFiles(i).name;
                fullFileName = fullfile(directory, baseFileName);
...
%                         samples{sampleCounter, 5} = extractStringValue(tline, 'Plugged');
                        ...
                        samples(sampleCounter, 1) = uint8(sscanf(tline(1, temp:temp+1), '%d')); %Hour
                        samples(sampleCounter, 2) = uint8(sscanf(tline(1, temp+3:temp+4), '%d')); %Minute
                        samples(sampleCounter, 3) = logical(sscanf(extractStringValue(tline, 'Charged'), '%d')); %Whether the phones have been fully charged
                        samples(sampleCounter, 4) = uint8(sscanf(extractStringValue(tline, 'Temparature'), '%d')/10); %Temperature in degrees Celsius divided by 10 (For applicability of the function uint8())
                        samples(sampleCounter, 5) = uint8(sscanf(extractStringValue(tline, 'BatteryLevel'), '%d')); %Battery level
                        samples(sampleCounter, 6) = logical(sscanf(extractStringValue(tline, 'Plugged'), '%d')); %Whether the phone has been plugged into charge
                    end
