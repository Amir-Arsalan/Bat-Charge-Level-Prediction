...
labeledDataset = [];
validDatasetsIndices = false(size(usersData, 1), 1);
...
for i=1:size(usersData, 1)
    ...
    validDatasetsIndices(i) = singleUserData(end, 1) < 100 && (temp(i, 2) < .25 || temp(i, 3) > .38);
end

validDatasets = cell(sum(~validDatasetsIndices), 2);
indices = find(~validDatasetsIndices);
for i=1:sum(~validDatasetsIndices)
...
if(mode == 1) %Tag each single record (No hierarchical model)
    ...
    %Some more cleaning
    tempIndx = allUsersData(:, 7) == 0 & allUsersData(:, 9) < 0;
    allUsersData(tempIndx, 7) = 1; %Setting all records having a negative charge/discharge rate to recharge status
    tempIndx = allUsersData(:, 9) > 0 & allUsersData(:, 7) == 1 & allUsersData(:, 6) < 97;
    allUsersData(tempIndx, 7) = 0; %Setting all records having a positive charge/discharge rate while the phone is being charged and the charge level is below 97 to discharge status
    tempIndx = allUsersData(:, 7) == 0 & allUsersData(:, 9) >= 30/(10/granularity);
    allUsersData(tempIndx, 9) = unifrnd(15, 25, sum(tempIndx), 1);
    ...
    % Start tagging
    tempIndx = allUsersData(:, 7) == 0 & allUsersData(:, 9) == 0; %Shutdown or idle - almost 50/50 for the simplest HMM model (no dependency on time, or any other variable)
    allUsersData(tempIndx, 10) = 1;
    
    tempIndx = allUsersData(:, 7) == 0 & allUsersData(:, 9) > 0 & allUsersData(:, 9) <= 0.35/(10/granularity); %Idle
    allUsersData(tempIndx, 10) = 2;
    
    tempIndx = allUsersData(:, 7) == 0 & allUsersData(:, 9) > 0.35/(10/granularity) & allUsersData(:, 9) <= 0.99/(10/granularity); %Low usage
    allUsersData(tempIndx, 10) = 3;
    
    tempIndx = allUsersData(:, 7) == 0 & allUsersData(:, 9) > 0.99/(10/granularity) & allUsersData(:, 9) < 2/(10/granularity); %Med-low usage
    allUsersData(tempIndx, 10) = 4;
    
    tempIndx = allUsersData(:, 7) == 0 & allUsersData(:, 9) >= 2/(10/granularity) & allUsersData(:, 9) < 4/(10/granularity); %Med usage
    allUsersData(tempIndx, 10) = 5;
    
    tempIndx = allUsersData(:, 7) == 0 & allUsersData(:, 9) >= 4/(10/granularity) & allUsersData(:, 9) <= 6.5/(10/granularity); %Med-high usage
    allUsersData(tempIndx, 10) = 6;
    
    tempIndx = allUsersData(:, 7) == 0 & allUsersData(:, 9) > 6.5/(10/granularity) & allUsersData(:, 9) <= 9.3/(10/granularity); %High usage
    allUsersData(tempIndx, 10) = 7;
    
    tempIndx = allUsersData(:, 7) == 0 & allUsersData(:, 9) > 9.3/(10/granularity); %Intense usage
    allUsersData(tempIndx, 10) = 8;
    
    %Recharge tags
    tempIndx = allUsersData(:, 7) == 1 & allUsersData(:, 9) > -0.5/(10/granularity); %Idle/Fully charged usage (more than 95% of the time)
    allUsersData(tempIndx, 10) = 9;
    
    tempIndx = allUsersData(:, 7) == 1 & allUsersData(:, 9) <= -0.5/(10/granularity) & allUsersData(:, 9) >= -3/(10/granularity); %Early charge state or getting fully charge (charge level usually > 90)
    allUsersData(tempIndx, 10) = 10;
    
    tempIndx = allUsersData(:, 7) == 1 & allUsersData(:, 9) < -3/(10/granularity) & allUsersData(:, 9) >= -6.5/(10/granularity); %Getting fully charge (charge level usually > 90)
    allUsersData(tempIndx, 10) = 11;
    
    tempIndx = allUsersData(:, 7) == 1 & allUsersData(:, 9) < -6.5/(10/granularity); % The device is being charged 
    allUsersData(tempIndx, 10) = 12;
